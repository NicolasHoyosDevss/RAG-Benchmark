...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Descarga el arbol de un documento en PageIndex y lo guarda como JSON local."
    )
    parser.add_argument("doc_id", help="ID del documento en PageIndex (ejemplo: pi-xxxx)")
    parser.add_argument(
        "--node-summary",
        action="store_true",
        help="Si se incluye, solicita resumen por nodo en la respuesta del arbol.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Carpeta de salida. Por defecto usa Data/processed en la raiz del proyecto.",
    )
    ...
    try:
        ...
        print(f"Error de PageIndex API al traer arbol: {exc}")
        raise
    ...
    print("Arbol guardado correctamente:")
    print(out_file)
